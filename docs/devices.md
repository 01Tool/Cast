...
| 2026-09-08 | Xiaomi (MediaTek) | Pad 7S Pro 12.5 | Intel AX210 `wlp4s0` | yes (scan) | — | — | — | **Treeland / Wayland, not a WFD verdict.** Scan listed this Pad. ScreenCast picker (DP-1) + Allow worked. Cast froze on “Select a screen to share…”. Dock system indicator: `正在共享屏幕至[]` (empty name; not `ot-cast-tray`). No P2P prompt on the Pad; no `pipewiresrc`. See [platform/wayland.md](platform/wayland.md) §Treeland smoke. X11 row above still stands. |

...
The app requests a ScreenCast session. The portal backend asks the compositor. Frames arrive as a PipeWire stream. The cast engine feeds that stream into the same encode/RTP path used on X11.
...
4. Keep the same WFD/P2P/encode stack as X11; only the frame source changes.
